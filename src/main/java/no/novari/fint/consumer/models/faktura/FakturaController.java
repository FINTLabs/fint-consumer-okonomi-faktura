package no.novari.fint.consumer.models.faktura;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Faktura", value = RestEndpoints.FAKTURA, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FakturaController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private FakturaCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Faktura cache is disabled.");
        }
    ...
    @GetMapping
    public FakturaResources getFaktura(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFakturaByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Faktura cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.GET_ALL_FAKTURA, client);
        ...
        Stream<FakturaResource> resources;
    ...
    @PostMapping("/$query")
    public FakturaResources getFakturaByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFaktura(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FakturaResources getFakturaByOdataFilter(
        ...
        Event event = new Event(
                ...
                FakturaActions.GET_FAKTURA, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FakturaResources();
    
        ArrayList<FakturaResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FakturaResource>>() {});
    ...
    @GetMapping("/fakturanummer/{id:.+}")
    public FakturaResource getFakturaByFakturanummer(
        ...
        log.debug("fakturanummer: {}, OrgId: {}, Client: {}", id, orgId, client);

        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.GET_FAKTURA, client);
        event.setOperation(Operation.READ);
        event.setQuery("fakturanummer/" + id);

        if (cacheService != null) {
            fintAuditService.audit(event);
            fintAuditService.audit(event, Status.CACHE);

            Optional<FakturaResource> faktura = cacheService.getFakturaByFakturanummer(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return faktura.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FakturaResource faktura = objectMapper.convertValue(response.getData().get(0), FakturaResource.class);
    ...
    public ResponseEntity getStatus(
        ...
        return statusCache.handleStatusRequest(id, orgId, linker, FakturaResource.class);
    }

    @PostMapping
    public ResponseEntity postFaktura(
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody FakturaResource body,
            @RequestParam(name = "validate", required = false) boolean validate
    ) {
        log.debug("postFaktura, Validate: {}, OrgId: {}, Client: {}", validate, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.UPDATE_FAKTURA, client);
    ...
    @PutMapping("/fakturanummer/{id:.+}")
    public ResponseEntity putFakturaByFakturanummer(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody FakturaResource body
    ) {
        log.debug("putFakturaByFakturanummer {}, OrgId: {}, Client: {}", id, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.UPDATE_FAKTURA, client);
        event.setQuery("fakturanummer/" + id);
