package no.novari.fint.consumer.models.fakturautsteder;
...
@Slf4j
@Api(tags = {"Fakturautsteder"})
@CrossOrigin
@RestController
@RequestMapping(name = "Fakturautsteder", value = RestEndpoints.FAKTURAUTSTEDER, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FakturautstederController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private FakturautstederCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private FakturautstederLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Fakturautsteder cache is disabled.");
        }
    ...
    @GetMapping
    public FakturautstederResources getFakturautsteder(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFakturautstederByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fakturautsteder cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.GET_ALL_FAKTURAUTSTEDER, client);
        ...
        Stream<FakturautstederResource> resources;
    ...
    @PostMapping("/$query")
    public FakturautstederResources getFakturautstederByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFakturautsteder(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FakturautstederResources getFakturautstederByOdataFilter(
        ...
        Event event = new Event(
                ...
                FakturaActions.GET_FAKTURAUTSTEDER, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FakturautstederResources();
    
        ArrayList<FakturautstederResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FakturautstederResource>>() {});
    ...
    public FakturautstederResource getFakturautstederBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.GET_FAKTURAUTSTEDER, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<FakturautstederResource> fakturautsteder = cacheService.getFakturautstederBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return fakturautsteder.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FakturautstederResource fakturautsteder = objectMapper.convertValue(response.getData().get(0), FakturautstederResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(fakturautsteder);
        }    
    }
