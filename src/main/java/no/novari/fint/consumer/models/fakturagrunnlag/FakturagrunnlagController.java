package no.novari.fint.consumer.models.fakturagrunnlag;
...
@Slf4j
@Api(tags = {"Fakturagrunnlag"})
@CrossOrigin
@RestController
@RequestMapping(name = "Fakturagrunnlag", value = RestEndpoints.FAKTURAGRUNNLAG, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FakturagrunnlagController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private FakturagrunnlagCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Fakturagrunnlag cache is disabled.");
        }
    ...
    @GetMapping
    public FakturagrunnlagResources getFakturagrunnlag(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFakturagrunnlagByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fakturagrunnlag cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.GET_ALL_FAKTURAGRUNNLAG, client);
        ...
        Stream<FakturagrunnlagResource> resources;
    ...
    @PostMapping("/$query")
    public FakturagrunnlagResources getFakturagrunnlagByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFakturagrunnlag(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FakturagrunnlagResources getFakturagrunnlagByOdataFilter(
        ...
        Event event = new Event(
                ...
                FakturaActions.GET_FAKTURAGRUNNLAG, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FakturagrunnlagResources();
    
        ArrayList<FakturagrunnlagResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FakturagrunnlagResource>>() {});
    ...
    @GetMapping("/ordrenummer/{id:.+}")
    public FakturagrunnlagResource getFakturagrunnlagByOrdrenummer(
        ...
        log.debug("ordrenummer: {}, OrgId: {}, Client: {}", id, orgId, client);

        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.GET_FAKTURAGRUNNLAG, client);
        event.setOperation(Operation.READ);
        event.setQuery("ordrenummer/" + id);

        if (cacheService != null) {
            fintAuditService.audit(event);
            fintAuditService.audit(event, Status.CACHE);

            Optional<FakturagrunnlagResource> fakturagrunnlag = cacheService.getFakturagrunnlagByOrdrenummer(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return fakturagrunnlag.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FakturagrunnlagResource fakturagrunnlag = objectMapper.convertValue(response.getData().get(0), FakturagrunnlagResource.class);
    ...
    public ResponseEntity getStatus(
        ...
        return statusCache.handleStatusRequest(id, orgId, linker, FakturagrunnlagResource.class);
    }

    @PostMapping
    public ResponseEntity postFakturagrunnlag(
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody FakturagrunnlagResource body,
            @RequestParam(name = "validate", required = false) boolean validate
    ) {
        log.debug("postFakturagrunnlag, Validate: {}, OrgId: {}, Client: {}", validate, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.UPDATE_FAKTURAGRUNNLAG, client);
    ...
    @PutMapping("/ordrenummer/{id:.+}")
    public ResponseEntity putFakturagrunnlagByOrdrenummer(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody FakturagrunnlagResource body
    ) {
        log.debug("putFakturagrunnlagByOrdrenummer {}, OrgId: {}, Client: {}", id, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, FakturaActions.UPDATE_FAKTURAGRUNNLAG, client);
        event.setQuery("ordrenummer/" + id);
